...
export default function Customer() {
  ...
  const updateQty = async (menuItemId, quantity) => {
    await fetch(`${API_BASE}/sessions/${sessionId}/cart/quantity`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ menuItemId, quantity }) })
    load()
  }
  ...
  const cancelOrder = async (orderId) => {
    await fetch(`${API_BASE}/sessions/${sessionId}/order/${orderId}/cancel`, { method: 'PATCH' })
    load()
  }
  ...
  return (
    <div className="container">
      ...
      <Routes>
        ...
        <Route path="orders" element={<OrdersPage session={session} onCancel={cancelOrder} onPay={pay} />} />
      ...
      <ul className="list space-y">
        {menu.map(m => (
          <li className="item row" key={m._id}>
            ...
            <button className="primary" onClick={() => onAdd(m._id)} disabled={!m.isAvailable}>
    ...
    <div className="space-y">
      ...
      {session.cart?.length ? (
        <>
          <ul className="list space-y">
            {session.cart.map(ci => (
              <li className="item row" key={ci.menuItem?._id || ci.menuItem}>
      ...
      {session.keepForLater?.length > 0 && (
        <>
          <h4>Keep for later</h4>
          <ul className="list space-y">
            {session.keepForLater.map(ki => (
              <li className="item row" key={ki.menuItem?._id || ki.menuItem}>
...
function OrdersPage({ session, onCancel, onPay }) {
  return (
    <div className="space-y">
      <h3>Orders</h3>
